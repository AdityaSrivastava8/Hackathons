...
class CaseRetriever:
    def _init_(self, cases_dir: str = "cases", db_path: str = "./chroma_db"):
    ...
    def _index_cases(self) -> None:
        """Reads all JSON files and indexes them into ChromaDB."""
    ...
    def search_similar_cases(self, suspect_query: str, top_k: int = 3) -> List[Dict[str, Any]]:
        """Queries the vector store for matching historical crimes with explicit type safety."""
        results = self.collection.query(query_texts=[suspect_query], n_results=top_k)
        
        matches: List[Dict[str, Any]] = []
        
        # Explicit checks to resolve Pyright reportOptionalSubscript errors
