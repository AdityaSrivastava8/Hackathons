...
def _domain_from_url(url: str) -> str:
    ...
    try:
        ...
        return (
            hostname.lower().removeprefix("www.")
            if hostname
            else ""
        )
...
def _wikipedia_search(
    ...
    relevance_words = (
        [w for w in str(keyword).lower().split() if w]
        + [
            "detective",
            "invest",
            "agency",
    ...
    if not app_password or not str(app_password).strip():
        ...
        return [{
            ...
            "error": (
                ...
                "Add GMAIL_APP_PASSWORD to Streamlit Secrets."
            )
        }]
